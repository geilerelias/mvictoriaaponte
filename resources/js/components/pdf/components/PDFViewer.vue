    ...
    <v-app>
        <v-container fluid class="ma-0 pa-0">
            <div class="pdf-viewer">
                <v-app-bar>
                    <v-btn text icon color="gray" @click.prevent.stop="back()">
                        <v-icon>mdi-arrow-left</v-icon>
                    </v-btn>
                    <header class="pdf-viewer__header">

                        <div class="pdf-preview-toggle">
                            <v-btn text icon color="gray" @click.prevent.stop="togglePreview">
                ...
                <PDFData class="pdf-viewer__main" :url="url" @page-count="updatePageCount"
                         @page-focus="updateCurrentPage" @document-rendered="onDocumentRendered"
                         @document-errored="onDocumentErrored">
                    <template v-slot:preview="{pages}">
                        <PDFPreview v-show="isPreviewEnabled" class="pdf-viewer__preview"
                                    v-bind="{pages, scale, currentPage, pageCount, isPreviewEnabled}"/>
                    </template>

                    <template v-slot:document="{pages}">
                        <PDFDocument class="pdf-viewer__document" :class="{ 'preview-enabled': isPreviewEnabled }"
